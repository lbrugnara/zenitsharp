// Copyright (c) Leonardo Brugnara
// Full copyright and license information in LICENSE file

using Fl.Engine.Symbols.Objects;

namespace Fl.Engine.Symbols.Types
{
    public class FlNamespaceType : FlType
    {
        private static FlNamespaceType _Instance;

        private FlNamespaceType(TypeDescriptor descr)
            : base(descr)
        {
        }

        public static FlType Instance => _Instance == null ? (_Instance = Initialize()) : _Instance;
        
        private static FlNamespaceType Initialize()
        {
            TypeDescriptor.Builder builder = new TypeDescriptor.Builder();

            builder

                // Class Name
                .WithName("namespace")

                .ExtendsFrom(FlObjectType.Instance)

                .WithActivator(o => FlNull.Value)
                ;

            // Build
            return new FlNamespaceType(builder.Build());
        }
    }
}
